...
export function createApp({ pool, origin, clientId = '', webRoot, verifyIdentity = verifyGoogle }) {
  ...
  for (const kind of ['math', 'spelling', 'vocabulary']) {
    app.post(`/api/${kind}/exercise`, async (req, res) => {
      const { grade, subject, newRound = false } = req.body ?? {};
      if (!Number.isInteger(grade) || grade < 1 || grade > 99 || typeof newRound !== 'boolean' ||
          typeof subject !== 'string' || !/^[a-z][a-z0-9_-]{0,63}$/.test(subject)) {
        return res.status(400).json({ error: 'Invalid course' });
      }
      const exercise = await startPractice(pool, req.session.id, grade, subject, kind, newRound);
      if (!exercise) return res.status(404).json({ error: 'Course unavailable' });
      res.json(exercise);
    });
    app.post(`/api/${kind}/answer`, async (req, res) => {
      const body = req.body ?? {};
      if (!uuid.test(body.id) || !uuid.test(body.exerciseId) ||
          !Number.isInteger(body.step) || body.step < 0 || body.step > (kind === 'math' ? 10 : kind === 'spelling' ? 1 : 0) ||
          (body.revision != null && (!Number.isInteger(body.revision) || body.revision < 0)) ||
          (kind === 'math' ? !Number.isInteger(body.answer) || body.answer < 0 || body.answer > 9999999
            : typeof body.answer !== 'string' || body.answer.length > (kind === 'spelling' ? 64 : 500))) {
        return res.status(400).json({ error: 'Invalid answer' });
      }
      const result = await answerPractice(pool, req.session.id, kind, body);
